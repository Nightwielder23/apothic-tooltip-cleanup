        ...
        for (int i = 0; i < elements.size(); i++) {
            ...
            try {
                ...
                for (Object inst : list) {
                    if (inst instanceof ItemStack gem && !gem.isEmpty()) {
                        valid.add(gem);
                    } else {
                        empty++;
                    }
                ...
                // mixed: rebuild the record with only the filled gems, then add the summary line after.
                ItemStack stack = (ItemStack) socketedAccessor.invoke(tc);
                if (socketComponentCtor == null) {
                    socketComponentCtor = tc.getClass().getConstructor(ItemStack.class, List.class);
                }
                Object newSocketComponent = socketComponentCtor.newInstance(stack, valid);
